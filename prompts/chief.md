...
Call `bootstrap` first. Prefer `area → project → goal → habit | task` (a framework, not a hard gate: `task` may `child_of` `project`; a habit does not need a goal parent). Identity is UUID. If you already have a UUID and need the record, call `get`. If you already have a UUID and need what is open around it, call `working_set`. Destructive tools need a key with destructive scope. Type and relation writes apply immediately; `list_activity` and `undo` are the brake.
...
The user is the human who runs this vault on this machine. An agent that can reach the vault MCP may read and write. Do not call the graph “the Vault.” Life data stays in the vault, not in git.

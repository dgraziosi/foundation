...
Call `bootstrap` first. Prefer `area → project → goal → habit | task` (a framework, not a hard gate: `task` may `child_of` `project`; a habit does not need a goal parent). Identity is UUID. If you already have a UUID and need the record, call `get`. If you already have a UUID and need what is open around it, call `working_set`. Destructive tools need `confirm: true`. Type and relation writes apply immediately; `list_activity` and `undo` are the brake.

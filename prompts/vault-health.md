...
- Backup path: (the path the operator named; skip if unset)
...
4. If a backup path is set, it is present and not older than the stale threshold. If unset, skip. Health checks that path; it does not create a dump.

...
export function registerUpsertTool(server: McpServer, pool: Pool, dataDir: string): void {
  defineTool(server, {
    name: "upsert",
    description:
      "Create or update a node (title, type, payload, data, status). Updates require base_updated_at (if-match). data JSONB-merges and is checked against the type json_schema. data.living.{system,id} is unique on live nodes (gmail | calendar | drive). living: null clears. data.code.{system,id} is unique on live nodes (github). code: null clears. data.url is an optional https href (no credentials); url: null clears; omit the key to leave url unchanged. Url is not unique. data.receipt.{system,id,kind} is unique on live nodes (done: gmail/sent or calendar/cleared). receipt: null clears. The server does not invent the receipt. Optional data.aliases is a user-authored string array (pass aliases: [] to clear; omit the key to leave aliases unchanged; punctuation-only values refuse and do not clear). Create (no id) runs the same lookup matcher: exact title or unique exact alias returns those candidates and does not write unless allow_duplicate is true. Token/fuzzy/compact matches warn and do not block. Create accepts idempotency_key. Optional actor / actor_label are stored on activity. Returns suggested_links (child_of / about / relates_to) from title FTS — proposals only; call link after the user accepts.",

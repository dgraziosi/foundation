...
export async function buildBootstrap(pool: Pool): Promise<BootstrapOutput> {
  ...
  return BootstrapOutputSchema.parse({
    ...
    how_to_extend: {
      ...
      working_set:
        "working_set returns the actionable working set around one live node: open work, dues, and the parent chain when the type has parent_types. Pass a live UUID. lookup a name, then this call. Open-only by default (include_completed for done). Depth 1 (max 2). Cap 40. A spine-root area uses a 14-day due window (America/New_York). Honest empty is items []. Unknown or deleted id returns { error, suggestion } like get. Live edges only — get stays one node, search stays vault-wide. Call get for payload or if-match; call link when the operator accepts a suggestion.",
    },
  });
}

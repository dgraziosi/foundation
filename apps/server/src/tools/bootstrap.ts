...
export async function buildBootstrap(pool: Pool): Promise<BootstrapOutput> {
  ...
  return BootstrapOutputSchema.parse({
    ...
    how_to_extend: {
      ...
      search:
        "search uses Postgres full-text search on title, node data (string values), and extracted inline payload text (HTML tags stripped; alt/title/aria-label kept; JSON string values, not the payload wrapper). Latin diacritics are folded so ASCII queries match accented tokens and vice versa (fiancee / fiancée). Filter by type. Soft-deleted nodes are excluded. Hits are id/type/title/snippet — call get to load payload and neighbor titles. If you already have a UUID, call get; do not search. An empty result is not a license to upsert a duplicate — try a shorter token or type filter; only upsert if the entity is new. Lexical recall only, not embeddings.",
    },
  });
}

...
4. Call `bootstrap` first. It returns the starter spine (`area → project → goal → habit | task` — preferred, not a hard gate for `task` → `project`), seeded types/relations, and how to extend the ontology.
   ...
   **Window:** read-only operator window at `/view` (same API key as MCP). On this machine: `http://127.0.0.1:8788/view`. From another machine on this vault: `http://<this-host>:8788/view`. Unlock, then Home (Recents, open tasks, type folders), Graph as its own page, Collection, and Detail as a page. Search is a rail overlay. The window does not write.

...
4. Call `bootstrap` first. It returns the starter spine (`area → project → goal → habit | task` — preferred, not a hard gate: a habit does not need a goal parent; `task` may `child_of` `project`), seeded types/relations, and compact rules. Read `foundation://guidance` resources for how to extend. Starter bot recipes are MCP prompts.

...
- [`docs/VIEWER.md`](docs/VIEWER.md) — operator window: surfaces, shell, paper and dark
...
4. Call `bootstrap` first. It returns the starter spine (`area → project → goal → habit | task` — preferred, not a hard gate for `task` → `project`), seeded types/relations, and how to extend the ontology.
   ...
   **Window:** read-only operator window at `/view` on this process (same API key as MCP). Unlock, then Graph, Search, Recents, Tasks, and an inspector. Paper first; dark is a second theme. Works from another machine on this vault. The window does not write.

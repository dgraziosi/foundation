...
- [`docs/AGENTS.md`](docs/AGENTS.md) — architect + writer + vault-keeping routine (the system that maintains Foundation)
- [`docs/VAULT_KEEPING.md`](docs/VAULT_KEEPING.md) — what vault-keeping is (and is not)
...
2. Start Postgres 16 (pgvector image; vector unused for now) and the Foundation server. Durable files go under `FOUNDATION_DATA` (default `./data`):
   ...
   **Stand up the keeper.** Compose is the product. The system that maintains it is an Architect agent plus a quiet weekday vault-keeping routine (not new MCP tools). Follow [`docs/AGENTS.md`](docs/AGENTS.md) — paste [`prompts/architect.md`](prompts/architect.md) and attach [`prompts/vault-keeper.md`](prompts/vault-keeper.md). What “healthy” means: [`docs/VAULT_KEEPING.md`](docs/VAULT_KEEPING.md).
...
3. Stand up the Architect + vault-keeping routine ([`docs/AGENTS.md`](docs/AGENTS.md))

...
The product ships [`scripts/backup-vault.sh`](../scripts/backup-vault.sh). Vault Keeper’s backup routine is [`skills/backup-vault/`](../skills/backup-vault/), which runs that script. Health checks the path; it does not dump. Dumps stay out of git.

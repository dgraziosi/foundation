...
- **Not a cloud-VM writer.** Run from Librarian on the computer that can reach box MCP.
...
`get` returns incident edges for a node you already know. For a full pass, read-only SQL on the box is OK.

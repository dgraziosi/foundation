...
- **`data.origin`:** optional `{ system, id }` for `gmail` | `calendar` | `drive` | `github`. Unique on **live** nodes. Identity (which person or object). Not sent mail and not a cleared event. Look up with `search` `{ origin }` (or `get` once you have the UUID). Foundation stores the ref only — **never fetch or mirror** those systems' bodies. There is no `kind` on origin.

...
- `receipt: { system, id }` looks up the unique live `data.receipt` (`gmail` | `calendar`). Kind lives on the stored node. Then `get`. A miss means that receipt is free to write.

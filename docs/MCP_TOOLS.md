...
- **`data.living`:** optional `{ system, id }` for `gmail` | `calendar` | `drive`. Unique on **live** nodes. Which living object. Not sent mail and not a cleared event. Look up with `search` `{ living }` (or `get` once you have the UUID). Foundation stores the ref only — **never fetch or mirror** those systems' bodies. There is no `kind` on living. There is no `url` on living. GitHub is `data.code`, not living.
- **`data.code`:** optional `{ system, id }` for `github`. Unique on **live** nodes. Which GitHub object. Look up with `search` `{ code }` then `get`. Store the ref only. Not a living Drive/Sheet.

...
- **Retype:** when `type` on an existing id changes, upsert revalidates every live incident edge against the new type (same `validateLink` ontology rules: `source_types`, `target_types`, `child_of` `parent_types`, `supports` spine). An edge that would no longer be allowed → `{ error, suggestion }` (unlink that edge with if-match, then retry, or keep the current type). The write does not drop edges. Same-type updates skip this check. if-match still runs first.

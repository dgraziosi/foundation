...
- `due_within_days` default **14**, max **90**. Used when the root is a **spine root** (spine `kind`, empty `parent_types` — seed `area`). Work rows stay if they are overdue, due on or before today + that many days, or undated **and** depth 1 (the root’s direct children). Timezone for “today” / overdue is the vault settings row (seed **America/New_York**), same as `search` `due`. Depth, hard cap, and this window default also read that row. Other roots skip this window; the cap still applies.
...
**Sort.** `role: "parent"` first, nearest parent then further ancestors. Then `role: "work"`: overdue first, then upcoming by the sort date, then undated (title as tie-break). Sort date is `due` when set, else `start`. Overdue is sort date before today in the vault settings timezone.
...
2. **About a person** (`walk.work: "about"`) — the root type is listed in an associative relation’s non-empty `target_types` (seed `about` → `person`) and is not a hierarchy parent. Walk those targeted relations and unconstrained associatives (empty source and target), plus relations whose `semantic_parent_slug` is in that set. A person has empty `parent_types`; this walk does not invent a hierarchy parent on them.
3. **Event-like** (`walk.work: "event"`) — the type has `start` and `end` field roles and is not a hierarchy parent (seed `trip`). Walk hierarchy children when any exist, and all associative relations (and their semantic children) as the work around the event — seed `task` cannot hang under `trip` unless `parent_types` says so.
...
**Read-only.** Live edges only. The tool does not write, link, or turn `suggested_links` into edges. Call `get` for the record or if-match; call `list_activity` `{ target }` for the diary; call `link` when the user accepts a suggestion.

**How this sits next to `get` and `search`.**

| Need | Call |
| --- | --- |
| One record (`payload`, `data`, edges, `suggested_links`, `updated_at`) | `get` |
...
- **Retype:** when `type` on an existing id changes, upsert revalidates every live incident edge against the new type (same `validateLink` ontology rules: `source_types`, `target_types`, hierarchy `parent_types`). An edge that would no longer be allowed → `{ error, suggestion }` (unlink that edge with if-match, then retry, or keep the current type). The write does not drop edges. Same-type updates skip this check. if-match still runs first.
...
- **`data.receipt`:** optional `{ system, id, kind }` after a mail or calendar act. `system` is `gmail` | `calendar`. `kind` is `drafted` | `sent` | `booked` | `moved` | `cleared`. Pairing is closed: `drafted` or `sent` with `gmail`; `booked`, `moved`, or `cleared` with `calendar`. Unique on **live** `system`+`id`. Kind is not part of that key. Same record may hold url and receipt. Writing a gmail or calendar receipt refuses if a different live node holds that url; writing that url refuses if a different live node holds the receipt. Mail receipts with no url still write. Drive url is unchanged. `receipt: null` clears. Missing is allowed. Incomplete, unknown, or unpaired values refuse. Look up with `search` `{ receipt: { system, id } }` then `get`. The server does not invent the receipt. Store the ref only — no mail or event bodies.
...
- `url: { system, id }` looks up the unique live Drive / Gmail / Calendar url. Which object, not done. A https string is not this filter; `tools/call` returns `{ error, suggestion }`. `tools/list` advertises `{ system, id }`, not a string. Use `data_equals: { url }` for that address.
- `repo: { system, id }` looks up the unique live `data.repo` (`github`). Which GitHub object, not done.
- `receipt: { system, id }` looks up the unique live `data.receipt` (`gmail` | `calendar`). Kind lives on the stored node. Then `get`. A miss means that receipt is free to write, unless a different live node already holds the url for that gmail or calendar id — write on that url owner instead.
- Hits are lean (id/type/title/snippet, plus `due` when `data.due` is set). Call `get` for the record and neighbor titles. Call `list_activity` `{ target }` for the diary. Call `working_set` when the hit is a root and you want the open work around it.
...
- **An empty lexical result is not a license to upsert a duplicate.** The `suggestion` says so. Try a shorter token or a type filter; only upsert if the entity is new. If you already have a UUID, call `get` for the record or `working_set` for the agenda. A url miss means you may upsert with that `url` `{ system, id }` (ref only). A repo miss means you may upsert with that `data.repo` (ref only). A receipt miss means you may upsert with that `data.receipt` (ref only). To resolve one or more entity names to UUIDs, call `lookup`.
...
- If you already have a UUID, call `get` for the record or `working_set` for the open work around it. Listing, url, repo, receipt, due filters, and payload search stay on `search`. Lexical recall only, not embeddings. No hidden nickname list. The usual path after a bound name is `working_set` with that id.
...
- `GET /view/blobs/:id` — same bytes, same store, for the window. Unlock cookie or Authorization header. Same attachment / scriptable-type rules. The cookie still does not unlock `/mcp` or `/blobs/:id`.

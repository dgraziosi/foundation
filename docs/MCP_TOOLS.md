...
| `search` | Find nodes by text query and/or filters (`type`, `status`, `under`, `since`, `link`, `repo`, `receipt`, `due`, `due_on_or_before`, `due_on_or_after`, `data_equals`). Query is optional when a filter is set. Hits are id/type/title/snippet plus `due` when set. |
...
- **`data.link`:** optional `{ system, id }` for `gmail` | `calendar` | `drive`. Unique on **live** records. Which Drive, Gmail, or Calendar object. Not sent mail and not a cleared event. `link: null` clears. Look up with `search` `{ link }` (or `get` once you have the UUID). Foundation stores the ref only — **never fetch or mirror** those systems' bodies. There is no `kind` on link. There is no `url` on link. GitHub is `data.repo`, not link. Leftover `data.living` writes refuse.
- **`data.repo`:** optional `{ system, id }` for `github`. Unique on **live** records. Which GitHub object. `repo: null` clears. Look up with `search` `{ repo }` then `get`. Store the ref only. Not a Drive/Sheet. Leftover `data.code` writes refuse. Cursor Origin is not a vault key and not a `repo.system` value.
- **`data.url`:** optional https address on any type. Sibling of link (and of repo). How the Viewer opens a file that stays the source of truth. Validated only when the incoming `data` patch includes `url`. `url: null` clears. Omit the key to leave url unchanged (including a leftover value). Incomplete, non-https, credentialed, or non-string values refuse. Not unique — uniqueness stays on link (or repo). Store the href only — no file body, no blob. `get` returns it on `node.data`. Find the Drive / Gmail / Calendar object with `search` `{ link }`, then `get`. Open leaves the window for that file.
- **`data.receipt`:** optional `{ system, id, kind }` after a bot sends mail or clears a calendar event. `system` is `gmail` | `calendar`. `kind` is `sent` | `cleared`. Pairing is closed: `sent` with `gmail`, `cleared` with `calendar`. Unique on **live** `system`+`id`, independent of link. `receipt: null` clears. Missing is allowed. Incomplete, unknown, or unpaired values refuse. Look up with `search` `{ receipt: { system, id } }` then `get`. The server does not invent the receipt. Store the ref only — no mail or event bodies.
...
- **In:** `{ query?, type?, status?, under?, since?, link?, repo?, receipt?, due?, due_on_or_before?, due_on_or_after?, data_equals?, limit? }`
...
- `link: { system, id }` looks up the unique live `data.link` (`gmail` | `calendar` | `drive`). Which Drive, Gmail, or Calendar object, not done.
...
- **An empty lexical result is not a license to upsert a duplicate.** The `suggestion` says so. Try a shorter token or a type filter; only upsert if the entity is new. If you already have a UUID, call `get` for the record or `working_set` for the agenda. A link miss means you may upsert with that `data.link` (ref only). A repo miss means you may upsert with that `data.repo` (ref only). A receipt miss means you may upsert with that `data.receipt` (ref only). To resolve one or more entity names to UUIDs, call `lookup`.
...
- If you already have a UUID, call `get` for the record or `working_set` for the open work around it. Listing, link, repo, receipt, due filters, and payload search stay on `search`. Lexical recall only, not embeddings. No hidden nickname list. The usual path after a bound name is `working_set` with that id.

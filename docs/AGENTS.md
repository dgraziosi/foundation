...
3. Nightly backup — [`.agents/skills/backup-vault/`](../.agents/skills/backup-vault/). Host script [`scripts/backup-vault.sh`](../scripts/backup-vault.sh) dumps localhost Postgres. The bot nags if the dump is missing or old. In-place restore on this vault is [`scripts/restore-vault.sh`](../scripts/restore-vault.sh).

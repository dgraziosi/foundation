...
3. Nightly backup — [`.agents/skills/backup-vault/`](../.agents/skills/backup-vault/). Host script [`scripts/backup-vault.sh`](../scripts/backup-vault.sh) dumps localhost Postgres. The bot nags if the dump is missing or old. In-place restore on this vault is [`scripts/restore-vault.sh`](../scripts/restore-vault.sh). Activity prune is the host script [`scripts/activity-prune.sh`](../scripts/activity-prune.sh). It reads `vault_settings.activity_retention_days`.
...
| [`.agents/skills/foundation-mcp/`](../.agents/skills/foundation-mcp/) | Vault MCP — which call to use. Inventoriable tool table is generated from the advertised inventory in `packages/schema`. |

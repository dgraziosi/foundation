...
- **Compare-and-swap:** update, `link`, `delete`, `merge`, `unlink`, and node or edge `undo` are if-match. Pass `base_updated_at` (or endpoint timestamps) from `get`. Compared at millisecond precision so a never-updated node (including rows that still store leftover microseconds from `now()`) can be written when the caller passes `updated_at` from `get`. If the node moved, the vault refuses with stale (get and retry) — a CAS miss is never “node not found.” A stale delete or merge leaves the nodes live. Two connections racing one node leave one winner and one mismatch.
- **Retype and inbound refs:** changing `type` on upsert revalidates live incident edges against the new type. An edge the current ontology would reject refuses with `{ error, suggestion }`; the write does not drop the edge and does not leave it invalid. `delete` refuses when a live record still points at that id through a declared `ref` field. Clear the field, then retry. Refs stay typed UUIDs on `data`, not edges.
- **Merge:** `merge(keep, drop)` is a sixteenth inventoriable tool. Same-type live nodes, destructive scope, confirm set to true, and if-match on both. One transaction rewrites drop’s live edges and declared refs onto keep, unions aliases, moves unique identity when only drop holds it, and soft-deletes drop. One reversible `merge` activity row carries the undo payload. Self-loops that would become keep→keep are dropped as a no-op. Different hierarchy parents refuse. Conflicting unique `url` / `repo` / `receipt` values refuse. Keep wins on title, status, payload, and other `data` keys. Blob bytes stay.
...
- The server stamps `actor` / `actor_label` on the activity row from the authenticated key. Viewer writes stamp the user. Clients cannot impersonate another bot.
...
When a bot drafts or sends mail, or books, moves, or clears a calendar event, the record holds `data.receipt { system, id, kind }`. Store the ref only. `system` is `gmail` | `calendar`. `kind` is `drafted` | `sent` | `booked` | `moved` | `cleared`. Pairing is closed (`gmail` with `drafted`|`sent`, `calendar` with `booked`|`moved`|`cleared`). Live records are unique on `system`+`id`. That index is separate from url. When both a live url and a live receipt exist for the same gmail or calendar id, they must be the same node. Look up with `search { receipt }`, then `get`. The server does not invent the receipt. Gmail and Calendar stay the source of truth — no bodies in the vault.

```mermaid
flowchart LR
  move["Bot drafts, sends, books, moves, or clears"]
...
The user opens a window on the view publish: `/view` (`http://127.0.0.1:8788/view`). Off-box is `VIEW_HOST=0.0.0.0` (`http://<this-host>:8788/view`). The person types the vault key. When `FOUNDATION_VIEW_KEY` is set, that is the vault key; MCP keys do not open the window. When unset, the house key (`FOUNDATION_API_KEY` / named keys) still opens it. Same graph — not a second store. Vite + React on this process. One chrome, a content host, three surfaces: Home (Today, Since you last looked, Recents, open tasks, type folders for types that have live objects), Collection (the type’s declared views, including graph), Detail (a page — not a docked inspector). A journal opens a writing page; title and markdown body may write (if-match, actor user). Other live records edit title, status, and declared scalar fields on detail (same if-match, actor user). Activity lists that record’s writes and can undo a reversible row. Trash lists soft-deleted records and restore returns them live. Search is a rail overlay. The rail is Home, Trash, and Search. Types carry hue and glyph; Viewer reads them, with a quiet fallback when missing. Off-box unlock and session use the vault key and `Path=/view` cookie when `VIEW_HOST=0.0.0.0`. The cookie does not unlock `/mcp`. Blob bytes from the window are `GET /view/blobs/:id` (unlock cookie or Authorization header). Agents still fetch `GET /blobs/:id` with the header. Contract: [`VIEWER.md`](./VIEWER.md).

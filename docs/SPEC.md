...
- `manage_type` can retire an unused authored type (`action: "retire"`, `confirm: true`). System seed types cannot be retired. Live nodes of that type refuse; leftover soft-deleted nodes follow type-create undo (`purge_deleted: true` or restore those deletes first). A type owns `fields` (the field template) and `views` (ordered view declarations: engine `id` plus optional `filter` / `sort` / `group`). `json_schema` is compiled from `fields` (`additionalProperties: true`; `needed` is not JSON Schema `required`). `default_view` is one of those view ids. Seed types already declare views (`task` defaults to `board`). The Viewer reads that contract; it does not infer views. System seed types may edit description, `fields`, and the query on views they already declare. Their slug, kind, parent_types, label, and ordered view **ids** stay locked. Authored types keep the wider patch, including the view id list.
...
- Read-only window at `/view` (same API key; not a second store). Compose publishes MCP / health / agent blobs on `127.0.0.1:8787` and `/view` on `8788` (`http://127.0.0.1:8788/view`; from another machine, `http://<this-host>:8788/view`). Unlock with the key, HttpOnly cookie `Path=/view`. After unlock: Home (widgets + a folder per live type), Graph canvas, Search, Recents, a view engine driven by the type’s view declarations and field roles, and an inspector. Collection applies that type’s query (Home **Open tasks** uses the same `task` default-view filter). Session **Show completed** widens an active status filter for this window only — it is not a graph write. Dark is first paint; Light and System are real choices. A stored `paper` choice reads as Light. The cookie does not unlock `/mcp` or `/blobs/:id`. Contract: [`VIEWER.md`](./VIEWER.md).

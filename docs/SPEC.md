...
These names are the current surface. Full parameters: [`docs/MCP_TOOLS.md`](./MCP_TOOLS.md). Advertised `tools/list` parameter docs come from Zod `.describe()` on each input field. Generated regions in that file and the foundation-mcp inventory table come from the same advertised inventory in `packages/schema`. Rewrite one record with `get`, `upsert`, and `list_activity`.

`bootstrap`, `search`, `lookup`, `get`, `working_set`, `upsert`, `delete`, `merge`, `link`, `unlink`, `inspect_ontology`, `manage_type`, `manage_relation`, `list_activity`, `undo`, `job`.

`bootstrap` is lean: spine, live types, live relations, compact rules. How-to-extend prose is MCP resources under `foundation://guidance`. Starter bot recipes are MCP prompts (`chief`, `vault-keeper`, `executive-assistant`). Resources and prompts are not a seventeenth tool.

- Destructive tools (`delete`, `merge`, `unlink`, `undo`, `manage_type` retire) need a key with destructive scope. Ordinary upsert and link do not. A key without that scope returns `{ error, suggestion }`. `merge` also needs confirm set to true.
...
- Updates (`upsert` with an existing id, `link`) and destructive node or edge writes (`delete`, `merge`, `unlink`, and `undo` of a node or edge row) are if-match: pass `base_updated_at` / endpoint timestamps from `get`. Compared at millisecond precision (same instant `get` returns). Mismatch → `{ error, suggestion }` (get and retry), never “node not found.” A stale delete or merge leaves both nodes live. Two connections racing one node leave one winner and one mismatch. Not a write-ACL. Changing `type` on upsert revalidates every live incident edge against the new type (and the neighbor’s current type). An edge the ontology would now refuse → `{ error, suggestion }` (unlink that edge with if-match, then retry). The write does not drop edges and does not leave an invalid leftover. `delete` refuses when a live record still holds a declared `ref` field pointing at that id → `{ error, suggestion }` (clear `data.<field>` with upsert and if-match, then retry). Those fields stay pointers, not edges. `merge(keep, drop)` is the sixteenth inventoriable tool: same-type live nodes, confirm set to true, destructive scope, and `keep_base_updated_at` / `drop_base_updated_at`. It rewrites drop’s live edges and declared refs onto keep, unions aliases, moves unique identity when only drop holds it, soft-deletes drop, and writes one reversible `merge` activity row. Undo of that row restores the prior graph while reversible. Keep wins on title, status, payload, and other `data` keys. Conflicting unique `url` / `repo` / `receipt` values refuse. Self-loops that would become keep→keep are dropped as a no-op. Different hierarchy parents refuse (`unlink` first). `link` accepts one edge or a capped `edges[]` (1–20). The whole batch validates, then one transaction writes all edges or none. One activity receipt per written edge. Each edge carries both endpoint timestamps; a later edge does not inherit CAS from an earlier edge that named the same node. Shared endpoints still use one agreed timestamp; missing or disagreeing timestamps refuse the batch. Linking does not change `node.updated_at`. `undo` still writes a compensating row when the invert is safe; that compensating row is not reversible. Type and relation undo have no node timestamp from `get`.
...
- Activity stores `actor` / `actor_label` from the authenticated key (who wrote). Clients cannot set those fields. Viewer writes stamp the user. Not a permission matrix. Every node write leaves a row (`create` / `update` / `delete` / `merge`) with `before` / `after` snapshots (`payload`, `data`, title, type, status; merge stores keep, drop, edge dispositions, ref retargets, and identity moves). Blob snapshots store `payload.blob_id` plus blob metadata, not file bytes. New rows stamp `schema_version` (today `1`). Older rows read as `1`. `list_activity` `{ target: <node id> }` is the diary for that node. Newest first. Default limit 50, max 200. `since` is a time window (`created_at >=`), not a page. After a full page, send `next` as `cursor`. `count` matches the same filters. Omit `fields` and `diff_only` for the full snapshot. `fields` is a subset of activity keys. `diff_only: true` returns only changed top-level keys on `before` / `after` (a one-sided null keeps the other snapshot). Undo still reads the stored snapshot, not the lean view. `get` does not include these rows. Host script [`scripts/activity-prune.sh`](../scripts/activity-prune.sh) deletes rows older than `vault_settings.activity_retention_days`. Bots may claim job name `activity-prune` before that pass. Not a new MCP write tool.
- `search` is Postgres FTS (title + `data` + extracted inline payload text; Latin accents folded). `query` is optional when `type`, `status`, `under` (child_of parent), `since`, `url`, `repo`, `receipt`, `due` (`overdue` | `today` in the vault settings timezone, seed America/New_York), `due_on_or_before`, `due_on_or_after`, or `data_equals` is set, so agents can list without a word. `data_equals` is JSONB equality on one or a few top-level `data` keys (not a column per key). Hits include `due` when `data.due` is set. Default `limit` 20, max 100. `count` is the total for those filters. After a full page, send `next` as `cursor`. `since` is a time window (`updated_at >=`), not a page. Empty `{}` and `{ cursor }` with no filter refuse. A string `search.url` refuses from `tools/call` with `{ error, suggestion }`. `tools/list` advertises `{ system, id }`, not a string. Not embeddings. No `list_nodes`.
...
These fields are already on the type. `json_schema` is compiled from the template (`additionalProperties: true`; `needed` is not JSON Schema `required`). Missing needed fields warn on upsert; `strict: true` refuses. When a key is present, upsert validates it. `null` clears. Extra `data` keys still write. `search` and `get` treat them as real fields. `get` returns the validated `data` and does not return blob bytes for a spend line.

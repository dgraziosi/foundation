...
Viewer Open stays `data.url` (https string, not unique). That string is not which Drive / Gmail / Calendar object. `search { url }` is not `data_equals` on the https string. A string `search.url` refuses with `{ error, suggestion }`.

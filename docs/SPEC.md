...
`bootstrap` is lean: spine, live types, live relations, compact rules. How-to-extend prose is MCP resources under `foundation://guidance`. Starter bot recipes are MCP prompts (`chief`, `vault-keeper`, `executive-assistant`). Resources and prompts are not a sixteenth tool.

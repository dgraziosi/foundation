...
- `manage_type` can retire an unused authored type (`action: "retire"`, `confirm: true`). System seed types cannot be retired. Live nodes of that type refuse; leftover soft-deleted nodes follow type-create undo (`purge_deleted: true` or restore those deletes first). A type may declare `views` (ordered view-engine ids) and `default_view`. Seed types already declare views (`task` defaults to `board`). The Viewer reads that declaration; it does not infer views.

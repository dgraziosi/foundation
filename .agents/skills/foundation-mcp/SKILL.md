...
- `search` — list or find without a bound id. After a full page, send `cursor`. Read `count`. `search` `{ url }` finds a Gmail, Calendar, or Drive object. `search` `{ repo }` finds a GitHub object. `search` `{ receipt }` looks up a mail or calendar receipt, then `get`. Same tool, not a new verb. `due: today` and `overdue` use the vault settings timezone (seed America/New_York). No settings tool.
- `upsert` — write or patch one record, or `nodes[]` (1–20) in one all-or-nothing call. **Always pass `type` on create and update** (server required; omit → validation error). Passing `payload` replaces that body. Updates need `id` plus `base_updated_at` from the latest `get` (`updated_at`). Creates use an idempotency key. `dry_run: true` returns would-be snapshots and writes nothing. Missing type `needed` fields warn; `strict: true` refuses. A bot writes `data.receipt` after draft, send, book, move, or clear. The server does not invent it. When a live url and a live receipt both exist for the same gmail or calendar id, write both on that one record. Changing `type` revalidates live incident edges and refuses if one would no longer be allowed (unlink first).
...
A record is what is true now, short. History stays in activity. To rewrite one record: `get` → `list_activity` `{ target }` → keep what still matters, invent nothing → `upsert` the same id with `type`, a short `payload`, and `base_updated_at`. Several new records may go in one `nodes[]` call. Not a background job. The server does not invent the body.

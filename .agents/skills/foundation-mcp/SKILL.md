...
- `upsert` — write or patch one record, or `nodes[]` (1–20) in one all-or-nothing call. **Always pass `type` on create and update** (server required; omit → validation error). Passing `payload` replaces that body. Updates need `id` plus `base_updated_at` from the latest `get` (`updated_at`). Creates use an idempotency key. `dry_run: true` returns would-be snapshots and writes nothing. Missing type `needed` fields warn; `strict: true` refuses. A bot writes `data.receipt` after send or clear. The server does not invent it. Changing `type` revalidates live incident edges and refuses if one would no longer be allowed (unlink first).

...
A **record** is the node — what is true now, short. **Activity** is that record's diary of writes. History stays in activity. Do not call the graph “the Vault.”
...
This pass rewrites records that moved. Vault health, nightly backup, weekly graph hygiene, and product updates have their own schedules. Graph hygiene stays the weekly report. Do not turn this pass into that report.
...
2. For each record, in order:
   ...
   3. Keep what still matters on that record. Invent nothing. Do not write a digest onto the record.

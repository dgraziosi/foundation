...
- `FOUNDATION_API_KEY` is in the environment. Do not print it.
...
- **HTTP accept.** `curl -sS -D - http://127.0.0.1:8788/view/unlock -H "content-type: application/json" -H "accept: application/json" -d "{\"api_key\":\"${FOUNDATION_API_KEY}\"}"`. Status `200`. Body `{"ok":true}`. `Set-Cookie` includes `foundation_key=` and `Path=/view` and `HttpOnly`. Redact the cookie value in evidence.
- **Session.** `curl -sS http://127.0.0.1:8788/view/api/session -H "Authorization: ApiKey ${FOUNDATION_API_KEY}"`. Status `200`. Body `{"ok":true}`.

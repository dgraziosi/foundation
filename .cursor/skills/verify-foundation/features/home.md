...
- **HTTP Home widgets.** `GET /view/api/digest` and `GET /view/api/recents?limit=5` and `GET /view/api/tasks?limit=5` and `GET /view/api/ontology` and `GET /view/api/journals/today` with the vault key (view-key-file when present). Keep a cookie jar across digest loads so the last-looked watermark persists. The unlock cookie authenticates; the watermark is set on the digest GET. The window caps the digest, Recents, and Open tasks at 5. Empty first-day: digest, recents, and tasks are `[]`. Today peek may be `{ "node": null }`. Ontology lists types; folders in the window are those with `count > 0`.
